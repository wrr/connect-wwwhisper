 ...
 * https://github.com/wrr/rack-wwwhisper
 * Functionality and test cases are identical. The only difference is
 * that the Connect middleware does not attempt to change public caching
 * to private caching for wwwhisper-protected content served over
 * HTTP. Serious wwwhisper deployments need to use HTTPS anyway, and
 * it seems that returning correct cache control headers should be
 * better left as a responsibility of the application.
 */
...
var AUTHCOOKIE_REGEXP = new RegExp(AUTH_COOKIES_PREFIX + '-[^;]*(?:;|$)', 'g');

function copy_auth_cookies(cookies) {
  return cookies.match(AUTHCOOKIE_REGEXP).join(' ');
}
